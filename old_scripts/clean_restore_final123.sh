...
# Clean Restore Final123 Script
# Completely clears all data and restores from final123.sql

echo "🧹 Clean Restore Final123"
echo "========================="
...
echo "Step 3: Starting only PostgreSQL..."
...
# Terminate all connections and drop database
echo "Step 5: Terminating all connections and dropping database..."
docker exec eve-chatting-platform_postgres_1 psql -U postgres -c "
SELECT pg_terminate_backend(pid) 
FROM pg_stat_activity 
WHERE datname = 'chatting_platform' AND pid <> pg_backend_pid();
"
...
# Create extension
echo "Step 6: Creating UUID extension..."
...
# Create ALL 8 tables WITHOUT foreign key constraints
echo "Step 7: Creating ALL 8 tables WITHOUT foreign key constraints..."
...
echo "Step 8: Verifying tables are empty..."
docker exec eve-chatting-platform_postgres_1 psql -U postgres -d chatting_platform -c "
...
echo "Step 9: Copying final123.sql to container..."
docker cp final123.sql eve-chatting-platform_postgres_1:/tmp/final123.sql

# Restore data from final123.sql WITHOUT foreign key constraints
echo "Step 10: Restoring data from final123.sql..."
...
# Add foreign key constraints AFTER data is restored
echo "Step 11: Adding foreign key constraints..."
docker exec eve-chatting-platform_postgres_1 psql -U postgres -d chatting_platform -c "
...
echo "Step 12: Verifying database restoration..."
docker exec eve-chatting-platform_postgres_1 psql -U postgres -d chatting_platform -c "
SELECT 'users' as table_name, COUNT(*) as count FROM users
UNION ALL
SELECT 'admin_users', COUNT(*) FROM admin_users
...
echo "Step 13: Final table count verification..."
docker exec eve-chatting-platform_postgres_1 psql -U postgres -d chatting_platform -c "
...
# Health checks
echo "Step 16: Performing health checks..."
...
# Final status
echo "Step 17: Final service status..."
docker-compose ps

echo ""
echo "🎉 Clean Restore Final123 Complete!"
echo ""
echo "📋 Service Status:"
...
echo "   ✅ ORIGINAL DATA RESTORED from final123.sql"
echo "   ✅ Foreign key constraints added"
echo "   ✅ No duplicate key errors"
echo ""
