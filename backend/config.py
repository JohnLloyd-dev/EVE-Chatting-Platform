...
class Settings(BaseSettings):
    ...
    ai_model_file: str = os.getenv("AI_MODEL_FILE", "openhermes-2.5-mistral-7b.Q6_K.gguf")
    ...
    # Accuracy-Optimized Generation Parameters
    ai_temperature: float = float(os.getenv("AI_TEMPERATURE", "0.1"))  # Lower temperature for more focused responses
    ai_top_p: float = float(os.getenv("AI_TOP_P", "0.85"))  # More conservative for accuracy
    ai_top_k: int = int(os.getenv("AI_TOP_K", "20"))  # Lower for more focused sampling
    ai_typical_p: float = float(os.getenv("AI_TYPICAL_P", "0.95"))  # Higher for better token filtering
    ai_tfs_z: float = float(os.getenv("AI_TFS_Z", "0.98"))  # Higher for better sampling
